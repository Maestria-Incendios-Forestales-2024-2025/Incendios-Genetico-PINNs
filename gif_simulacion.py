...
for t in range(num_steps):
    ...
    if t % 20 == 0:
        ...
        if t*dt <= 24:
            ax1.set_title(f"Tiempo = {t*dt:.2f} horas")
        else:
            ax1.set_title(f'Tiempo = {t*dt/24:.2f} días')
        ...
        # Guardar el frame como archivo de imagen temporal
        filename = f'frame_{t}.png'
        plt.savefig(filename)
        plt.close()

        # Agregar el nombre del archivo a la lista de filenames
        filenames.append(filename)

# Crear el GIF usando las imágenes guardadas
with imageio.get_writer('simulacion.gif', mode='I', duration=0.1, loop=0) as writer:
    for filename in filenames:
        image = imageio.imread(filename)
        writer.append_data(image)
        os.remove(filename)  # Eliminar el archivo de imagen temporal
